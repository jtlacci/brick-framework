...
- `run.py` contains the module's only repository-internal entry point: `run`.
...
- Do not expose adapters or `src/` functions to sibling modules.
...
- Keep `runner/__init__.py` and `runner/tests/__init__.py` so sibling modules with the same test filenames retain distinct import paths during discovery.
